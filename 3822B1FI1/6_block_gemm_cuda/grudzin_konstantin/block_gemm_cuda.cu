...
__global__ void MultKernel(const float* A, const float* B, float* C, int n) {
...
__host__ std::vector<float> BlockGemmCUDA(const std::vector<float>& a,
    ...
    cudaMemcpy(A, a.data(), memory, cudaMemcpyKind::cudaMemcpyHostToDevice);
    cudaMemcpy(B, b.data(), memory, cudaMemcpyKind::cudaMemcpyHostToDevice);
    
    int grid = (n + SIZE - 1) / SIZE;
    dim3 gridSize(grid, grid),
         blockSize(SIZE, SIZE);
    
    MultKernel<<<gridSize, blockSize>>> (A, B, C, n);
    cudaMemcpy(result.data(), C, memory, cudaMemcpyKind::cudaMemcpyDeviceToHost);
